...
const getTimeList = ({ bookings, selectedDay }: GetTimeListProps) => {
  return TIME_LIST.filter((time) => {
    ...
    const hasBookingOnCurrenctTime = bookings.some(
      (booking) =>
        booking.date.getHours() == hour && booking.date.getMinutes() == minutes,
    )

    if (hasBookingOnCurrenctTime) {
      return false
    }
...
const ServiceItem = ({ service, barbershop }: ServiceItemProps) => {
  ...
  const handleDataSelect = (date: Date | undefined) => {
    setSelectedDay(date)
  }
    ...
    <>
      <Card>
        <CardContent className="flex items-center gap-3 p-3">
          <div className="relative max-h-[110x] min-h-[110px] min-w-[110px] max-w-[110px]">
            <Image
              alt={service.name}
